...
module.exports = class FastlaneGenerator extends Base {
  initializing() {
  ...
  writing() {
    this.fs.copyTpl(this.templatePath('fastlane/*'), this.destinationPath('fastlane'));
    this.fs.copyTpl(this.templatePath('env'), this.destinationPath('fastlane/.env'));
    this.fs.copyTpl(this.templatePath('environment/*'), this.destinationPath('src/environment'));
  ...
  _extendGradle() {
    ...
    // Change the app id
    config = config.replace(/applicationId ".*"/, 'applicationId _applicationId');
  ...
  _activateManualSigning() {
    let config = this.fs.read(
      this.destinationPath(`ios/${this.answers.projectName}.xcodeproj/project.pbxproj`)
    );

    // Manual signing
    config = config.replace(/ProvisioningStyle = Automatic;/g, 'ProvisioningStyle = Manual;');
    config = config.replace(
      /(TargetAttributes = {(?:\n.*?)+?TestTargetID = )(.+?)(;\n.*\n)(.+)/m,
